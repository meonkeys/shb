...
These should match the public names and point to ((LAN))-only private IP addresses.
This way you can use the same domain names inside and outside your LAN and your Let's Encrypt certs will work.
...
When mario is able to connect to the server and provision it successfully, you'll see output similar to this:
...
It may take that minute or two for ((Traefik)) to set up ((Let's Encrypt)) ((HTTPS)) encryption certificates, so don't worry if you get invalid cert warnings at first.
...
Each service has a "`Setup`" section detailing command(s) to start it.
...
. If you have a dynamic IP address, set up a dynamic ((DNS)) update client container.
. Stand up a mail relay container.
