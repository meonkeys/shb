...
If you don't have an internal ((DNS)) server, you can add more hostname to IP address mappings like we did in <<SSH setup>>.
For example:
...
Manually mapping IP addresses to hostnames with a hosts file is handy for initial setup and maintenance when the (internal) DNS server fails.
...
If you waited and are still getting invalid cert warnings from your browser when you try to visit `\https://traefik.example.com`, read the Traefik log messages carefully and also see <<Encryption certificates>> for troubleshooting steps.
