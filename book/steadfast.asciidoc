...
This book will help you efficiently learn the fundamentals of self-hosting.
You'll learn how to face its challenges while experiencing its benefits first-hand.
These lessons are applicable at home, at work, and in your community.
...
Saving a copy of data someone else is hosting for you is fine.
...
(((router, network device)))
That is: remote access to your services (hence: your ((LAN)) and your data), from basically anywhere (the WAN), coming in through your router/firewall.
One means of allowing WAN access is by port forwarding ((HTTPS)) traffic through your router/firewall to your ((Nextcloud)) server.
...
((ZFS)) (originally: Zettabyte File System) is our one-stop shop for efficiently and safely making hard drives available for our data.
...
The root partition is ((ext4)) (not ZFS) so we can:
...
** we don't need to
** the OS will live outside our ZFS folders
...
((Ubuntu)) LTS (long-term support) releases are the most stable so we'll use one of those.
...
Once your server is online following <<Deploy>>, use mario to configure and start services.
...
Individual services will be named based on this, e.g. `media.example.com`.
...
Oh great, _another_ config file?
Don't worry, there's only one value to change: `ansible_become_password`.
That should be set to the password you entered when you created the user account on your server, way back in <<Deploy>>.
...
Here's an example client config template for ((OpenSSH)):

.mario server snippet in OpenSSH client config (admin computer)
[%unbreakable,text]
----
Host mario_server
...
While you are waiting, tail the logs with:
...
. Re-provision with mario to copy this change on the server.
. Re-start Wallabag with `sudo docker-compose --file /root/ops/wallabag/docker-compose.yml up -d`.
...
router::
  Network device used to handle traffic at the boundary between networks such as our WAN and LAN. This is more formally a border router, so forgive my using the term loosely. A SOHO router typically also provides various other functions including switching, firewalling, and Wi-Fi. See: port forward. A Traefik router is something different: this is a software logic connecting entrypoints to services. See <<Traefik architecture>>.
runtime::
