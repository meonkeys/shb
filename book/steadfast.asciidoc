...
_Steadfast_ specifically mentions 22.04, the LTS release from April 2022.
22.04 is stable today, and will receive updates https://ubuntu.com/about/release-cycle[until April 2032], promising many more years of stability until _Steadfast_ must be revised.
...
That should be set to the password you entered when you created the user account on your server, way back in <<OS install>>.
...
# for accessing services from browsers and devices
...
`192.168.1.100` is a placeholder.
To get the IP address of your server, log in and run `netplan status`.
You must be able to `ping` that IP address from your admin computer.

Manually mapping IP addresses to hostnames with `/etc/hosts` is a temporary measure.
...
Replace `mario_server` with the name of your server, and `your-username` with the account username on your server.
...
Adjust that as necessary for the SSH client you use.
(((provision)))
Test it by running `ssh mario_client`.
You'll see something like this:

.first SSH to server (admin computer)
[source%unbreakable,text]
----
...
You can go ahead and type kbd:[yes+Enter]; this is very likely safe to ignore for a LAN-only server you just created.
If you want to be super careful, run this on the server and confirm the fingerprints match:

.show SSH host public ECDSA key (server)
[source%unbreakable,bash]
----
ssh-keygen -lf /etc/ssh/ssh_host_ecdsa_key.pub
----

Either way, once you get `ssh mario_server` working, run `provision.sh` again.

IMPORTANT: mario not only encourages you to use public key authentication for remote SSH connections, it forcibly disables password-based SSH authentication on its first `provision.sh` run.
...
When you're done, stop tailing the logs with kbd:[Ctrl+c].
...
You'll be able to add and remove services as desired.
...
It can be daunting to self-host.
