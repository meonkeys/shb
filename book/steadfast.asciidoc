...
It gets a __host__name during <<OS install>>, but this one doesn't matter so much for a _Steadfast_ server.
...
Check with your DNS provider about adding the DNS or "`host`" records for this purpose (e.g. `A` and `CNAME` records).
