...
"`🚀 server`" indicates this particular example Bash script snippet is intended to be run on the server described in this book.
...
[source#traefik-and-nextcloud-config,yaml]
----
...
For more robust data storage, you can add a couple of ((HDD))s and manage them with ((ZFS)).
...
It gets a hostname during <<OS install>>, but this one doesn't matter so much for a _Steadfast_ server.
I recommend getting a domain name and naming all your services using subdomains (e.g. `cloud.example.com`).
...
GB::
  Gigabyte. 1,000^3^ (1,000,000,000) bytes for HDDs, or 1,024^3^ (1,073,741,824) bytes for RAM.
good, fast, and cheap::
