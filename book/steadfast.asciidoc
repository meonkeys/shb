...
Since mario uses ((Docker Compose)), another important config file is `docker-compose.yml`.
...
For example:
** Has a popular and well-maintained Docker image.
** Has instructions for integrating with ((Docker Compose)) and ((Traefik)).
...
The guide below walks through setup of a simple pool of two mirrored drives.
