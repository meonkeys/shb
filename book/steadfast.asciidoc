...
I was pleasantly surprised by containers following my varied earlier experiences with ((VM))s.
...
.. Set up a “Profile”; note the username and password.
...
. Run mario on your ((admin computer)) to provision your server. See <<Run mario>>.
...
Once our server is online following <<Deploy>>, we can use mario to configure and stand up services.

Please download the <<Source code>>.
It'll be helpful to have the source code handy so you can follow along as you read through this chapter.
...
Or perhaps your real server _will be a VM_ -- that'll work fine too.
...
IMPORTANT: FIXME - instructions need reordering here.

.FIXME - these instructions are out of place and poorly repeated later
****
First make sure you can SSH into your server from your ((admin computer)).
It helps if the server has a handy name rather than just an IP address.
See <<Internal DNS>> for help giving your server a name on your ((LAN)).
If you want DNS outside your LAN, see <<Public DNS>> (this is a more advanced topic).

Generate a SSH authentication keypair if you don't already have one.
Use `ssh-copy-id` or similar to copy your public SSH key onto your server.
****

Follow these instructions, re-running `provision.sh` as indicated until you see output like this:
...
Thanks to Bryan Daisley, Rob Floberg and all the Deadbeat Dads for your invaluable feedback.
