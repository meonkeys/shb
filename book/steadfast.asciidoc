...
Mitigating at multiple layers (OS firewall, service, WAN boundary) is called "`defense in depth`".
It's a great idea.
...
I recommend starting with one ext4 filesystem and, optionally, one ZFS filesystem.
ext4 is the default filesystem for Ubuntu, a stable and simple choice.
