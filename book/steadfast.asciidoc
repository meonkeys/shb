...
.. When you set up a Profile, note the username and password. You'll need these in <<Connect to server>>.
...
Here's a guide for every setting in your `config`.
That file also contains prodigious comments and is the most up to date, so please see it as well.

// FIXME - simplify to required values only

`DNS_API_PROVIDER`::
Leave this as `duckdns` unless you want to use Route 53.
See <<Server domain name>>.
`DUCKDNS_TOKEN`::
This is basically your secret key.
Grab this value from the Duck DNS website.
Leave unchanged if you are using Route 53.
`R53_DNS_API_KEY`::
Enter your key here if you aren't using Duck DNS.
`R53_DNS_API_SECRET`::
Enter your secret here if you aren't using Duck DNS.
`DNS_RESOLVER_EMAIL`::
...
Individual services will be named based on this, e.g. `media.example.duckdns.org`.
`MARIO_EMAIL_SENDER`::
Used for Gmail SMTP server login.
This may be set to the same value as `DNS_RESOLVER_EMAIL`.
`MAIL_RELAY_PASSWORD`::
Used for Gmail SMTP server login.
If you use multi-factor auth, use an app password here.
`DEFAULT_PHONE_REGION`::
Set this to your 2-digit ISO 3166-1 country code.
This is used to squelch a warning in Nextcloud.
`TZ`::
Set this to your timezone using a tz database name such as `America/Los_Angeles`. See https://en.wikipedia.org/wiki/List_of_tz_database_time_zones.
`LAN_ONLY_ALLOWED`::
This is a range of IP addresses allowed to connect to your server.
Use CIDR notation.

TODO - add `hosts.yml` tips here.

// How does an admin (and mario) connect to your server with SSH?::
// See <<Connect to server>> for how to do this.

Once you're on your server and ready to mario it into submission, skip to <<Provision complete>>.

=== Server domain name

mario expects to be able to use ((Duck DNS)) or Amazon Route 53 for responding to DNS (Domain Name System) queries and setting up ((HTTPS)) web traffic encryption.
You can either use a free domain name from ((Duck DNS)) or buy a domain name from a registrar.
