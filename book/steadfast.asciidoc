...
Second, a learning tool called mario.
mario is a small set of scripts and configuration files to help you set up and maintain your own server.
...
NOTE: Admonitions like this draw your attention to auxiliary information.
...
<<_mario>> is a tool you can use to take care of some of the fiddly details of setting up a server.
Here is what mario provides, starting with a high level overview.
...
mario requires a 64-bit ((Ubuntu)) server with at least 2GB memory and 30GB storage.
...
It is much easier to SSH into the server and run a one-off command rather than change configuration files and re-run mario.
...
* edit FIXME: `docker-compose pull && docker-compose up -d` (at least for those not auto-upgraded -- see <<Watchtower>>)
****

// FIXME - change read-write example with docker-compose, we'll
// but what? almost every read-write op should be done with mario/Ansible!
...
Since mario uses ((Docker Compose)), another important file is `docker-compose.yml`.
...
(((router, Traefik)))
In the mario source code (or the snippets appearing later), look at the `docker-compose.yml` files for Traefik and Nextcloud, which include:
...
.. When you set up a user account ("`Profile`"), note the username and password. You'll need these in <<Connect to server>>.
...
# Create encrypted dataset in "main" pool. This is the "parent" dataset, we can easily add more later and they'll all be encrypted.
...
// FIXME - reword; each service has Maintenance section, can automate as desired w/Watchtower
...
Create ((backups)) following the 3-2-1 rule of thumb: make *3* backups.
Store at least *2* local copies on different media.
Have *1* remote backup.

Check the backups regularly to make sure they work.
...
Once your server is online following <<Server startup>>, use mario to configure and start services.
...
When you run `docker-compose`, you must first be in a folder containing a `docker-compose.yml` file.
...
[source,bash]
----
sudo docker-compose --file /path/to/myservice up -d
----

mario installs a program called `dc` which can be used like so:
...
# equivalent to
# sudo docker-compose --file /path/to/myservice up -d
sudo dc myservice up -d

# will fail with a permission denied error
dc myservice up -d
----

TIP: For brevity in this text, I may omit `sudo`.
Remember that sudo is always required to run some commands, including `docker-compose`.

==== Start reverse proxy

Stand up the ((reverse proxy)) first.
Connect to your server and start ((Traefik)) with `dc traefik up -d` (actually: `sudo dc traefik up -d`).
...
[source,bash]
----
services="watchtower wallabag jellyfin nextcloud scratch"
...
You can increase the Traefik log verbosity by setting `--log.level=DEBUG` in `roles/services/templates/ops/traefik/docker-compose.yml`, re-provision, and re-start Traefik.
...
. Check logs for all containers related to Netcloud with `sudo docker-compose --file /root/ops/nextcloud/docker-compose.yml logs -f`.
