...
[#image-service-stack]
.Layers of a mario-built system. From the bottom we have hardware: bare metal, filesystem: ext4 for / and ZFS for /data, OS: Ubuntu LTS 64-bit server, container runtime: Docker, containers: Nextcloud file sharing app, Jellyfin media server, Wallabag article reader.
