...
(((Nextcloud, overview)))
A _Steadfast_ homelab needs convenient file sharing and synchronization.
...
// FIXME - check all values in this section
...
A few steps in <<Detailed setup>> are necessary to be able to see the folders in Nextcloud.

To see these folders in Jellyfin, add two media libraries:
...
image::watchtower.png[align="center",scaledwidth=25%]
...
These social features may reorient a user from productive creation to unintended consuming.
Scratch can be self-hosted without the social features.
In fact, that's the simplest way to self-host it.
This is a great option if your users want to focus on creating while online and being social in person.
...
Upgrade and replace service containers with `dc nextcloud pull && dc nextcloud up -d`.
This will pull the most recent image of https://github.com/nextcloud/docker with the `stable` release tag.
